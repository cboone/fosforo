...
//! rather than a fifth site. Raising that is a deliberate decision that wants
//! an ADR, not a side effect of picking a clock. Nothing warns in the meantime:
//! the declarations below are Zig's, so no deprecated header attribute is ever
//! read.
