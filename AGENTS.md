...
- **Every ordering-critical declaration is canaried against its own source**, through `src/canary.zig`, which reads a file's own text at comptime and fails `zig build test` if a stated ordering has moved. Fifteen such tests across seven files cover the ring, `std.Io`, `Gate`, `Pending`, the editor's counters, `Mailbox`, `Watcher.halt`, the shader watcher's bookkeeping, the race harnesses' controls and the declaration sweep; [concurrency and canaries](docs/notes/concurrency-and-canaries.md) is the inventory. Adding a cross-thread declaration without one is the omission this exists to stop (ADR 0016's #91 amendment, #90, #91).
...
- **Run the worktree's own `scripts/measure-trace`, never a copy from anywhere else, and do not believe `--periods` off a post-#58 capture.** It restates constants a branch may have moved, so another copy reports confident numbers against the wrong mapping; and its lit threshold is still an absolute energy that velocity weighting spread over two decades ([#126](https://github.com/cboone/fosforo/issues/126)), so the fast parts of a trace read as dark. The centroids are unaffected. See [measuring a capture](docs/notes/measuring-a-capture.md).
