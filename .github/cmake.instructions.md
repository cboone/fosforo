...
The measurements behind the rules below are in [`docs/notes/build-system.md`](../docs/notes/build-system.md) and [`docs/notes/signing-and-notarization.md`](../docs/notes/signing-and-notarization.md), including why the signing command must stay the last `POST_BUILD` command on the AUv2 target. They are restated here rather than replaced by those links because these instructions fire on an `applyTo` glob and a note cannot.
...
- **`CMAKE_CXX_STANDARD`, `CMAKE_OSX_DEPLOYMENT_TARGET` and `CLAP_WRAPPER_DOWNLOAD_DEPENDENCIES` are set here because clap-wrapper does not set them when consumed as a subproject.** They look redundant and are not. The deployment target is `11.0` and must stay in step with `build.zig`, `macos/Info.plist` and `packaging/distribution.xml`, which is four sites and not three: the last is the installer's `allowed-os-versions` and fails in the opposite direction from the other three.
