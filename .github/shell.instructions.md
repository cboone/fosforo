...
The full argument behind the `shfmt` rules below, including why passing any parser or printer option makes it discard `.editorconfig` wholesale, is in [`docs/notes/linters.md`](../docs/notes/linters.md). It is restated here rather than replaced by that link because these instructions fire on an `applyTo` glob and a note cannot.

**The exception is `scripts/measure-trace`, and this file's glob for** `scripts/**` **captures it regardless.** It is Python, run through `#!/usr/bin/env -S uv run --quiet --with pillow --with numpy python`, and it is the only non-Bash file under either directory. `ruff` lints it, `ruff.toml` rather than `.editorconfig` carries its style, and it is invisible to every selector below, because `shfmt -f` finds shell files by shebang and does not recognize that one. **Nothing in the list below applies to it.**
