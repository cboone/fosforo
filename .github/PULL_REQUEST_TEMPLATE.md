...
- [ ] All new and existing tests pass in every mode CI runs (`zig build test`, `zig build test-safe`, `zig build test-release`)
...
- [ ] If I touched anything under `.github/workflows/`, `actionlint` is silent, run with `shellcheck` on `PATH` (without it, it skips every `run:` block and still exits 0)
