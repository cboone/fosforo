...
**A hot-reloaded shader's binding indices are validated and its `TraceUniforms` layout is not**, which is [#77](https://github.com/cboone/fosforo/issues/77) closing the half a text scan can reach and stating the half it cannot. The `TraceUniforms` layout test and the `scripts/measure-trace` constants test still read the **embedded** copy at comptime, which is correct and must stay so: a test about a file that can differ between a build and a frame would say nothing, and making `zig build test` read the filesystem is exactly what the next paragraph refuses. What #77 changed is that `bindingIndexIn` takes its source as a parameter, so one `bindings` table serves both the comptime test over the embedded copy and `noteBindings` over whatever came off disk.

A mismatch is **warned about and swapped in anyway**, on a measurement rather than a preference: three copies of the shipped shader with one index moved apiece, one per index space, each completed its draw with the unbound argument reading as zeros, giving a flat trace, a black background and no trace at all. So refusing would trade a wrong picture the next save fixes for a reloader that declines to reload, and this decision comes first here because it is the one a reviewer reaches for the other way round. `iface.ShaderStats.binding_mismatches` makes the check assertable rather than trusted, and `zig build smoke-gpu` plants a shader that binds elsewhere and asserts both halves: the counter moved, and the reload still happened.

The half left open needs a readback of a _reloaded_ shader. MSL computes its own struct offsets, so a `TraceUniforms` field added or reordered on one side only leaves text that still describes the struct correctly and draws a plausible trace at the wrong scale. [#51](https://github.com/cboone/fosforo/issues/51) built the readback and still does not cover it, since `smoke-trace` measures the embedded copy and nothing reloads during a trace run.
