...
Both sentences above are the decision as it was taken and are superseded rather than corrected: #51 added a third half that is required too, and #72 discharged the condition the second names. The amendments at the foot of this document carry both. The _table_ is corrected in place, on the precedent the #38 amendment set for a stale list, because it enumerates what exists rather than stating a decision.
...
**The third plant had to be strengthened before it tripped.** A `readWindow` that stopped once _one frame_ had been presented still uploaded a second window, because the count is captured after the first wait and the assertion asks only that it advanced. Stopping once one window had uploaded is the defect the assertion actually names, and that one trips it. What the assertion catches is a path that stopped, not one that slowed.
...
The rule since #38 has been that _a Metal resource which is not a buffer_ must be checked against `leaks` with a planted leak. That is now **any allocation**, because the narrower version excluded the case that turned out to be broken.
...
**One consequence arrives free and is worth stating before it surprises someone.** The leak step carries `continue-on-error` and no `if:` condition, so a failing AppKit half now _skips_ it rather than running it, and the notice reads `leaks: skipped`. That is this document's own assertion order enforced one level up: a leak report over a run that fell over partway through describes a process that never reached the teardown being measured.
...
| One compiling without the right functions    | That the bytes _on disk_ reached the compiler                         |
...
A swap that leaked its outgoing pipeline states is caught by `smoke-leaks` with **no new instrument**, because this document's own #38 amendment measured that an `MTLRenderPipelineState` _is_ visible to `leaks` where a buffer and a texture are not. This is the first time the rule set after #38 has been discharged by a measurement already taken rather than by a new one, and it is why no counter for pipeline states was added.
...
The #51 amendment above said "the geometry assertions read the _accumulation_ rather than the picture precisely because #60 rewrites only the latter". That turned out to be exactly right, and it is worth recording as a hit rather than leaving it as a claim: of the nine cases in the trace half, **seven were untouched** by a change that replaced the resolve outright, moved the drawable to a different pixel format, and deleted the trace's colour. `checkDecay` in particular would have been destroyed had it read the picture, since the sRGB curve does not preserve a ratio.
...
**Same instrument, same blindness, so it joined the existing counter rather than getting a ninth seam operation**, and `liveAccumulationTextures` was renamed `liveTextures`. The distinction worth carrying is that the rule is about _measuring_ separately, not about _counting_ separately: what a caller asserts is that no texture leaked, and one number answers that. The rename was not cosmetic — with the old wording a leaked palette reported "7 accumulation textures were never released", which sends the reader to the wrong file. Confirmed by planting again after the rename: `error.TexturesLeaked`.

### What the harness still cannot see, and #60 sharpened it

The offscreen path reads `getBytes:` off a texture and never involves CoreAnimation. So `smoke-trace` settles what the _hardware_ does — it read `RGBA(5, 5, 8, 255)` for a background written as `float3(5, 5, 8) / (255 * 12.92)`, which is only true if the render-output stage encoded on write and `getBytes:` decoded nothing — and it says nothing whatever about what the **compositor** does with a `_sRGB` layer whose colorspace is nil. A screenshot of a running host is the only instrument for that, which is this document's position restated one layer up: the thing the harness cannot see is still the picture, and it is now the picture _on a screen_ rather than the picture at all.

## Amended by issue #89: the offscreen wait's bound was a spin count, not a duration

The finding recorded under #51, that "offscreen frames stop at three without a wait ... The harness yields instead", stands and is still what the code does. What it did not record is the _bound_ on that yielding, and that bound was wrong in a way nothing in this document's instrument set was arranged to see.
...
## Amended by issue #92: the trace half's judgements leave the harness, and the plant table stops being prose

The #51 amendment above made this argument and won it about the feature extraction. It stopped there. `src/gpu/measure.zig` reads a rendered trace back as numbers and has its own tests and its own beam model; the thirteen `check*` functions that decide whether those numbers are the ones the constants demand stayed in `src/smoke.zig`, which had 2,240 lines and zero test blocks and is not in `zig build test`'s module graph at all. The extraction was tested and the **expectations** were not, which is where the tolerances, the loops and the guards against going vacuous all live.

The judgements now live in `src/gpu/verdict.zig`, which names no Metal type and imports `std`, the seam, `measure.zig` and `palette.zig`. `src/smoke.zig` keeps the driving halves, and the seam between them is two value types: `measure.Image` over the accumulation, which already existed, and `verdict.Picture` over the resolved bytes, which did not. Everything above them owns a `gpu.Renderer` and nothing below them can name one.

**Nothing about what the harness reports changed, and that is the test of a move.** `zig build smoke-trace`'s transcript is byte-identical to the run before the refactor, line for line, on a machine whose figures already matched CI's. The judges print through a `say` gated on `!builtin.is_test`, which is `src/clap/log.zig:109`'s idiom applied for this document's own reason: a test binary has to stay silent, and gating the printer rather than moving the printing to the driver is what kept the bodies a move instead of a rewrite.

### What planting established that reading did not

The tests were written as the historical plants, and then the judges were planted against to see whether the tests discriminated. Twenty-two weakenings, one at a time. That second pass is where everything below came from, and none of it was visible in the first.

**Two vacuity holes, and the sharper one had been passing for two issues.** `checkDecay` and `checkDecayIsInRealTime` divided the measured peak by a first peak nothing checked. A blank readback gives `0 / 0`, and `nan > 0.02 * want` is **false**, so a run in which nothing was drawn at all read as a healthy fade and reported one. Planted with the guard removed, the judge returns `void` where the test now demands a refusal. The second is smaller: `checkPeriods` indexed an `undefined` array written only inside a `switch` over a literal frequency list, which was sound exactly while that list stood.

**One reported hole was not one, and only a plant could tell.** This issue was filed saying `expectClose`'s tolerance "divides by `b`, which is undefined if a future caller passes zero". It performed no division, and at zero it reduced to exact equality, which is the _right_ answer for a relative tolerance, since nothing is relatively close to zero but zero. The old spelling was planted back and the test written for it passed, so the test asserted nothing. The real defect was that it read its scale from one of its two arguments, so swapping them could change the verdict; the discriminating case is `a = 2`, `b = 1` at a tolerance of 0.75, which the old form refuses and the new one accepts.

**Encoding a plant is not the same as covering it.** Two arms survived their own weakening. The centreline plant sat at 0.01 of full scale, which is 2.4 times a whole backing pixel, so widening the twentieth-of-a-pixel bound twentyfold changed no verdict: the arm was asserting that the error had a detectable sign rather than that it was inside a twentieth of a pixel. The background plant sat one byte off the palette's value at zero, which the per-pixel loop refuses anyway within its own one-level slack, so removing the dedicated check traded one fault for another rather than letting a wrong picture through. **This is the #38 rule one level up**: a tolerance wide enough to absorb a systematic error hides one, and so does a plant far enough outside a bound to be caught by something else.

**And one arm cannot fire at all.** `movingCore` refuses a channel gap of zero or less before refusing a gap under 24. Every gradient's largest tint component is exactly 1.0 and the shipped palette's other two are below it, so the second arm already refuses everything the first would. It stays, because it is what survives a palette whose non-lead tint reaches 1.0, and it now says so at the source. A dead defensive arm is worth its line; a dead one that reads as live is not.
...
## Amended by issue #118: an outcome nothing had ever executed, and the arm that now does

The #93 amendment above says of the six-row outcome table that "which outcome happens where is still outside every test binary", and answers it with canaries, which read source and prove nothing about behaviour. What it does not say, because nobody had asked, is that one of the six rows had never been **executed** by any instrument in this repository at all — not by `zig build test`, which cannot compile the call site; not by `smoke-gpu`, whose six fallback arms all drive `probe` and therefore the opening path; and not by `smoke-appkit`, whose five live-swap arms were two compiler refusals, two good edits and a recovery. `watch_unreadable` was reachable only by a developer whose shader file happened to become unreadable while an editor was open.

That is a different kind of gap from the three this ADR already catalogues. A false negative is a defect one instrument cannot see. Structural uncoverability, #93's kind, is code no test binary compiles. This is neither: the code compiles in a plain `zig build`, an instrument that could reach it exists, and **no arm was ever written to steer it there**. Call it an unvisited branch, and note what makes it hard to notice — every counter it moved was read by something, so nothing looked absent.

It mattered because the row was wrong. It credited `fallbacks`, which the seam documents as "times the embedded copy was used", on the one path that keeps the shader already running and says so on stderr. An arm would have had to assert the counter to catch it, and there was no arm.

**`hotReloadPhase` now has six, and the fifth is the one this records.** Making the watcher's read fail while the change is still noticed has exactly one door: `Watcher.poll` stats first and hands the result to `reload.Watch.look`, so deleting the file or making it unstattable fails the _stat_, which `look` maps to the "learned nothing" case that never reaches a read. A file that stats cleanly and then will not fit is what is left, and `src/gpu/metal/shader.zig` refuses a read that fills its buffer rather than truncating it. The fixture is therefore exactly `shader.max_bytes` bytes — the ambiguous length the refusal exists for, rather than a size past it.

**It closes a second thing that was covered by nothing**, and this one was invisible from the same angle: `shader.read`'s size refusal. That file's tests are all pure and it asserts that none of them opens anything, so the branch had no caller outside a host. The arm is now the only thing that runs it.

**Planted at one byte under the boundary, the file compiles, swaps in, and the arm fails as `ShaderNeverReloaded`** — so the arm asserts the boundary rather than merely passing near it, which is the #38 rule this ADR keeps restating. `smoke-gpu`'s transcript is unchanged across the whole change, which is the evidence that narrowing `fallbacks` reached no further than the watcher; `smoke-appkit`'s changed by this arm's one diagnostic line and the amended closing line.

**The weakness in arm 4 recorded under #93 stays open and is now arm 4 of six.** It waits on `rejected >= n + 1` while the previous arm's broken file is still being re-rejected four times a second, so it can pass without the renamed shader ever being read. Unrelated to which counter a read failure moves, and deliberately not fixed inside a change whose evidence is that `smoke-gpu` did not move.
