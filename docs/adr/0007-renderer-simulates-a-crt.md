...
**The emergent hot core is real, and it is now executed rather than asserted.** `zig build smoke-trace` drives one deposit and thirty and reads the picture back: `RGB(102, 207, 119)` and `RGB(255, 255, 255)`. Nothing draws a core, exactly as this ADR predicted; what produces it is the top of one monotone ramp being reached only where the beam dwelt.
...
**Two departures from the wording above, both deliberate.** This ADR says "shaded by perpendicular distance from the centerline", which is the infinite line: no caps, no joints, no overlap. What is built measures distance to the *segment*, which gives round caps, and that buys three things a perpendicular distance does not. Joints are covered with no wedge gap outside a turn, and a degenerate segment collapses to a disk rather than to a NaN.

The edge columns are closed too, and **planting defects established that the caps are not the mechanism**. `zig build smoke-trace` reads 960 of 960 columns lit at the level that failed [#38](https://github.com/cboone/fosforo/issues/38), on a single frame, where every earlier reading came from an accumulated screenshot persistence could have been masking. But cutting the caps off entirely — butt joints, no extension along the segment — still reads 960 of 960. What covers column zero is the quad's *body*: the first segment spans a whole pixel horizontally, so it contains that pixel's centre. A line's endpoint was a point and had to satisfy the rasterizer's diamond-exit rule to light anything. The fix is that a quad has area at all, which is a weaker and more robust claim than the one this paragraph first made.
...
## Amendment, 2026-09-05: the single relationship is built ([#58](https://github.com/cboone/fosforo/issues/58))

**Velocity weighting is in, and the caveat above is spent.** Each segment's deposit is multiplied by `h / (h + len)`, where `h` is the beam's half-width and `len` is the segment's screen length in backing pixels. Both asymptotics are the physics: proportional to `1 / len` for a segment longer than the beam, constant below it, because a beam moving less than its own width does not reduce a pixel's dwell.

**The constant is derived rather than chosen, and that is what makes the relationship assertable.** A capsule's integral of the biweight is `(16/15)·h·len` along its length plus `(π/3)·h²` for the two caps, so the weight that holds a segment's *total* deposit constant is `1 / (1 + 1.019·len/h)`, which this is to within 2%. The physical statement is therefore executable: a segment carries one sample interval of beam time whatever it does with it, so the energy it lays down is constant and only its distribution changes. `zig build smoke-trace` drives four slopes across two sample densities and finds the total flat to **0.94%**, against a factor of **197** unweighted.

**What the display gained is not what this ADR predicted, and the difference is worth recording.** [ADR 0019](./0019-brightness-is-a-fixed-transfer-function.md) held `white_headroom` provisional on the prediction that this would widen the dwell range by an order of magnitude. Measured offscreen, turning point against zero crossing, at an amplitude of 0.5 in a 20 ms window:
...
The order of magnitude is real at 1 kHz and is not real at 100 Hz, and every figure the prediction was made from came from a 100 Hz sine. That signal's fastest crossing is only 1.88 times the speed of its turning point, since the sweep runs at 96,000 px/s against the trace's 152,681 px/s, so nothing can widen it and 1.84 is the beam telling the truth about it.

**What actually changed is that the range now discriminates between signals.** Before, 100 Hz and 1 kHz both read about 1.35: the display said the same thing about a slow tone and a fast one, which is exactly what this ADR calls throwing the information away. Now they read 1.84 and 7.50, and the difference between them is the information. "A kick's slow sub-tail glows solid while its sharp click smears dim" is a claim about *contrast between parts of one signal*, and that is the number that moved.

**One consequence for `TraceUniforms.density`, which is a defect rather than a design.** That term is meant to be the time a segment stands for, which depends on the window length alone; it is computed as the segment pitch, which ties it to the editor's width as well. Velocity weighting responds to that same geometry in the opposite direction, so the two no longer cancel cleanly. Measured offscreen on one 960-sample window, as peak energy on a flat trace and on a full-scale zigzag: a 960x540 drawable reads 1.198 and 0.0136, a 480x540 one reads 1.193 and 0.0110, and a 480x270 one reads 1.193 and 0.0219. So the *contrast* moves with the window by up to 1.6x, where the physics says a beam covering half the distance in the same time should be twice as bright everywhere.

Those three geometries are all at or below one sample per logical point, and the clamp makes the other regime sharper. Above one sample per point `density` is pinned at 1 and stops compensating for anything, while the velocity weight keeps dividing by a segment length that is now the pitch. A flat trace's peak across editor widths, at 960 samples: **1.59, 2.00, 1.58, 1.58** before this change at 480, 960, 1920 and 3840 points, against **1.19, 1.20, 0.68, 0.43** after. The ratios are `h / (h + pitch)` to three figures, so it is the weight doing exactly what it says; what it means is that a slow trace's dependence on editor width went from 1.27x across that range to **2.79x**. Pre-existing, deepened here, and reachable by dragging a 48 kHz session's editor wide. #58 deliberately did not touch it: it is a second relationship, it needs a reference geometry to fix absolute brightness against, and it reopens a correction #57 measured and settled. Filed as [#125](https://github.com/cboone/fosforo/issues/125). A resize during playback in a host settled its priority: none of this is visible, because the whole effect lives at the dim end between about green 29 and green 42, so the defect is real, measurable and cosmetically inert.
