...
Critically, the phosphor look is **not nostalgia**. The two things it produces, brightness that varies along the trace and a persistent halo, are direct readouts of the signal's statistics: where it dwells, and how much it varies. A single-trace plot throws both away. The reason to reproduce the physics is that the physics _is_ information, and on a GPU it is cheap.
...
**The emergent hot core is real, and it is now executed rather than asserted.** `zig build smoke-trace` drives one deposit and thirty and reads the picture back: `RGB(143, 224, 154)` and `RGB(255, 255, 255)`. Nothing draws a core, exactly as this ADR predicted; what produces it is the top of one monotone ramp being reached only where the beam dwelt. The first figure read `RGB(75, 189, 96)` when [#60](https://github.com/cboone/fosforo/issues/60) measured it and was corrected at [#96](https://github.com/cboone/fosforo/issues/96), because [#57](https://github.com/cboone/fosforo/issues/57) made one deposit 2.6 deposits of overlap.
...
**Two departures from the wording above, both deliberate.** This ADR says "shaded by perpendicular distance from the centerline", which is the infinite line: no caps, no joints, no overlap. What is built measures distance to the _segment_, which gives round caps, and that buys three things a perpendicular distance does not. Joints are covered with no wedge gap outside a turn, and a degenerate segment collapses to a disk rather than to a NaN.

The edge columns are closed too, and **planting defects established that the caps are not the mechanism**. `zig build smoke-trace` reads 960 of 960 columns lit at the level that failed [#38](https://github.com/cboone/fosforo/issues/38), on a single frame, where every earlier reading came from an accumulated screenshot persistence could have been masking. But cutting the caps off entirely — butt joints, no extension along the segment — still reads 960 of 960. What covers column zero is the quad's _body_: the first segment spans a whole pixel horizontally, so it contains that pixel's centre. A line's endpoint was a point and had to satisfy the rasterizer's diamond-exit rule to light anything. The fix is that a quad has area at all, which is a weaker and more robust claim than the one this paragraph first made.
...
**The profile is the biweight, `(1 - u²)²`, and its compact support is doing more work than its shape.** It peaks at exactly 1.0 on the centreline, so a single segment still deposits an energy of one and the white point's derivation from the dwell asymptote is untouched. It reaches zero _with zero slope_ at the quad's edge, so there is no seam. And an unlit pixel holds exactly 0.0, which is what lets the resolve check keep its background assertions and lets `src/gpu/measure.zig` read a beam's centre by summing a whole column. A Gaussian is the profile a real beam has and has none of those three properties.
