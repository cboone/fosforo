...
>
> **What changed is the property this ADR actually cares about: the range now discriminates between signals.** Before, 100 Hz and 1 kHz both read about 1.35, so the transfer function was mapping the same narrow band whatever was playing. That is the failure mode the provisional marking was really about, and it is gone. `white_headroom` stays at **0.8**, now supported by numbers rather than by the derivation alone: at 1 kHz it puts a turning point at green 191 and a crossing at 98 over a background of 5, and the offscreen harness still takes thirty deposits to exactly 255. The by-eye confirmation in a host on real material is the one thing outstanding, and it is a confirmation rather than a tuning.
