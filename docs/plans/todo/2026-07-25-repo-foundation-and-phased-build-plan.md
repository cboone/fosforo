...
**The phase is serial where a host is involved and nowhere else, and that distinction is newer than the rule above.** Verification belongs to every issue, and it was once true that almost every issue's verification needed a host, which made the whole phase queue behind one. [#51](https://github.com/cboone/fosforo/issues/51) is what changed it, by adding a check that renders the shipping pipeline into a texture and so needs a device and no window at all. **[#22](https://github.com/cboone/fosforo/issues/22) did not**, which is worth stating because it looks as though it should have: `CLAP_PATH` makes two concurrent REAPERs possible and the project has deliberately declined to use them, for the reasons in the paragraph below the table.

So there are three lanes. **The first admits one issue at a time, by rule rather than by contention; the other two admit any number, including alongside the first.**

| Lane                            | What its verification needs                               | What it contends with                                                                                                                                                     | Open issues                       |
| ------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
...
**The Audio Unit is the one thing that did not become a lane, because it is a location.** There is no `AU_PATH`, macOS does not follow a symlink into a worktree, and the installed component therefore belongs to whichever worktree copied last (ADR 0018). Two issues needing the *Audio Unit* still cannot be in flight at once. [#65](https://github.com/cboone/fosforo/issues/65) is the case worth naming, because it reads like an Audio Unit issue and is not one: its decisive check is a positive control on **other vendors'** plugins in Logic, which needs Logic and never loads this project at all, so it does not take the install path from anyone.
