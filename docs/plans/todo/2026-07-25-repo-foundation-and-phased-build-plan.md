...
**Three are deferred questions rather than risks to this plan**, which
...
7. Tonemap pass plus palette lookup into an sRGB drawable, producing an emergent white-hot core inside a colored bloom without ever drawing a core explicitly. **Done** ([#60](https://github.com/cboone/fosforo/issues/60), [ADR 0019](../../adr/0019-brightness-is-a-fixed-transfer-function.md)). The core is executed rather than claimed: `smoke-trace` drives one deposit and thirty and reads `RGB(102, 207, 119)` and `RGB(255, 255, 255)` off the picture. Four palettes ship as Zig data uploaded into a lookup, so the GPU and the model that checks it read one table.
...
Filed against the [Phase 3 milestone](https://github.com/cboone/fosforo/milestone/3). Unlike phases 1 and 2, **these are not a chain**: several depend only on #55, and five belong to no numbered step at all. That last number is the phase's own reporting rather than scope creep, and it grew from one to five as the verification rule below did its work.
...
| [#58](https://github.com/cboone/fosforo/issues/58) | 5    | Velocity weighting. Depends on #57, since there are no segments before there is geometry                                                                                                                                                                                                                                                                                                                   | Done   |
...
| [#79](https://github.com/cboone/fosforo/issues/79) | none | The bright vertical line a transport stop draws. A host hands over silence rather than stopping `process`, so the ring holds a step to zero and the beam draws it faithfully. **Check it against #58 before building anything here**, since velocity weighting is the exact relationship it violates, and it may close as covered rather than fixed                                                        | Open   |
| [#80](https://github.com/cboone/fosforo/issues/80) | none | Assert offscreen that one fixed function maps energy to colour, whatever produced the energy. **Rewritten once already**, because peak energy legitimately varies with level: a slower trace dwells, so the property is the transfer function's constancy across frames rather than the peak's constancy across signals                                                                                    | Open   |
| [#83](https://github.com/cboone/fosforo/issues/83) | none | Ten bits and a wider colour space. #57's skirt took `checkResolve`'s worst channel from 0 off to 1 off across 518,400 pixels, which is the quantiser becoming visible rather than a defect. The real cost is `iface.Readback.picture` and the three instruments built on 8-bit bytes, not the shader                                                                                                       | Open   |
...
**Three resources are exclusive, and they are exclusive for three different reasons.** Keeping the reasons apart is the value of the table, because it says which constraints can be argued with and which cannot.
...
**One clause in that argument has expired, and the rule survives it.** "The component was always exclusive" was doing double duty: it justified the one-host-stream rule on the grounds that half the verification could not parallelize whatever the CLAP did. **That half is no longer half.** Counting host mentions in the plans for the four most recent renderer issues, #55 names Logic nine times and REAPER nine, while #60, #56 and #57 name Logic **zero** times each and REAPER six, three and two. Three consecutive renderer issues verified in REAPER alone, because #55 was the one that allocated textures and therefore wanted the AUv2 path, where clap-wrapper destroys an editor rather than hiding it. So the honest justification for one host stream is the shared install path and the window server, not the Audio Unit's share of the work. The rule is still worth keeping, for the reason the paragraph above this one gives: the chain it would relax is dependency-serial anyway.
...
| **Machine and install path** | The verification half of #58, #59, #79, #83 and #34                     | Do not *run* two issues at the same moment. No effect on what may be branched, reviewed or merged           |

The resulting order is **#55**, then **#64**, then **#63**, then **#22**, then **#51**, then **#61**, then **#60**, then **#56**, then **#57** — all nine done — then **#58**.

**Beside #58, in the other two lanes.** [#77](https://github.com/cboone/fosforo/issues/77) is overdue rather than upcoming, which is the one piece of sequencing this plan got wrong rather than merely left stale: its own body says it bites first at #57, #57 has landed, and #58 moves bindings again if velocity weighting adds a uniform. [#62](https://github.com/cboone/fosforo/issues/62)'s algorithm half can be written into the reserved `src/dsp/decimate.zig` at any time and collides with nothing, since #57 touched no file under `src/dsp/` at all. [#30](https://github.com/cboone/fosforo/issues/30) and [#65](https://github.com/cboone/fosforo/issues/65) need nothing this phase is holding. **And the verification program below**, [#89](https://github.com/cboone/fosforo/issues/89) to [#99](https://github.com/cboone/fosforo/issues/99), of which ten are in the free lane. [#89](https://github.com/cboone/fosforo/issues/89) went first of everything in this paragraph, because it was why `main` went red, and it is **done**; the remaining ten are unblocked and can be taken in any order.

**Behind #58, in order.** [#79](https://github.com/cboone/fosforo/issues/79) first, and it may close as *covered* rather than fixed: velocity weighting is the exact relationship its bright transport-stop line violates, so #58 should be checked against it before anything of its own is built. Then **#59**. Then #62's wiring, with or after #59, because that is where the point count stops being bounded by the sample rate alone. Then [#83](https://github.com/cboone/fosforo/issues/83) after both, so the banding it measures is the banding that will ship rather than today's.
...
**What the review found is not "write more tests".** The suite is 205 named tests, about a quarter of the Zig source, and the instruments are layered with a written theory of what each one cannot see, which is the strongest thing in the repository. The gaps cluster in one shape instead: the question "would I know if this broke?" was answered once, by hand, and written into prose rather than into anything that re-runs. ADR 0013 and ADR 0016 both record planted defects as acceptance criteria; neither leaves behind a check that fails if the planted defect returns.

| Issue                                              | Work                                                          | What it needs                            | Status |
| -------------------------------------------------- | ------------------------------------------------------------- | ---------------------------------------- | ------ |
| [#89](https://github.com/cboone/fosforo/issues/89) | Bound the trace half's frame wait by time, not by yields      | `smoke-trace`, so a device and no window | Done   |
| [#90](https://github.com/cboone/fosforo/issues/90) | Canary the `std.Io` constructor and three unguarded orderings | `zig build test`                         | Open   |
| [#91](https://github.com/cboone/fosforo/issues/91) | Race `Gate` and `Pending` under Thread Sanitizer              | A Linux runner, so CI                    | Open   |
| [#92](https://github.com/cboone/fosforo/issues/92) | Make the trace half's judgements pure, and test them          | `smoke-trace`, so a device and no window | Open   |
...
| [#97](https://github.com/cboone/fosforo/issues/97) | Eleven small, named, missing assertions                       | `zig build test`                         | Open   |
...
| [#99](https://github.com/cboone/fosforo/issues/99) | Lint the workflows                                            | Nothing on this machine                  | Open   |

**Three of them were urgent in a way the rest are not, and the first has landed.** [#89](https://github.com/cboone/fosforo/issues/89) was why `main` went red: `smoke-trace` is a required check and it gave up against a bound expressed in scheduler yields rather than wall time, three seconds into a step that takes four when it passes. It is now a two-second wall-clock deadline, and `main` has been green since `6f0a860`. [#90](https://github.com/cboone/fosforo/issues/90) is the largest gap closed by the fewest lines, because [ADR 0015](../../adr/0015-adopt-std-io-single-instance.md) is a non-negotiable that a one-token edit defeats silently. [#92](https://github.com/cboone/fosforo/issues/92) is the largest gap: `src/smoke.zig` is 2,181 lines with zero tests and is not in `zig build test`'s module graph at all, so the thirteen `check*` functions carrying this project's hardest claims are verified by nothing that re-runs. #57 and #89 added 283 lines and two more checks to that file since the review, none of them tested.
...
| Unit tests  | `zig build test`, 205 named tests across `src/`. **Debug only**, while the shipped bundle is `--release=fast`; [#94](https://github.com/cboone/fosforo/issues/94) adds the mode that ships                                                                                                                                                                                                                                                                                                               |
...
| Pixels      | `zig build smoke-trace` renders the shipping pipeline into a texture and asserts the mapping, the rail, period counts, the resolve, the decay and the velocity weighting. Required in CI                                                                                                                                                                                                                                                                                                                 |
...
| CI          | Green on `macos-latest` and `ubuntu-latest`; `clap-validator` clean; gitleaks, TruffleHog and `typos` clean. One unintentional failure in the recorded history, on `0e1ddf5`, when the required `smoke-trace` step gave up against a bound counted in scheduler yields; [#89](https://github.com/cboone/fosforo/issues/89) replaced it with a wall-clock deadline and `main` has been green since `6f0a860`. Nothing lints the workflows themselves ([#99](https://github.com/cboone/fosforo/issues/99)) |
