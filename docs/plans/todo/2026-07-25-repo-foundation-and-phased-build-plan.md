...
| [#77](https://github.com/cboone/fosforo/issues/77) | none | Check a hot-reloaded shader's binding indices. Split out of #61, which left it open deliberately. `bindingIndexIn` takes its source as a parameter, so one `bindings` table serves the comptime test and a reloaded file alike. **Warns rather than refuses**, on a measurement: an unbound argument reads as zeros, so it costs a wrong picture rather than a wedge. Cannot see `TraceUniforms` drift     | Done   |
...
| [#99](https://github.com/cboone/fosforo/issues/99) | Lint the workflows                                            | Nothing on this machine                  | Done   |
...
| CI          | Green on `macos-latest` and `ubuntu-latest`; `clap-validator` clean; gitleaks, TruffleHog and `typos` clean. One unintentional failure in the recorded history, on `0e1ddf5`, when the required `smoke-trace` step gave up against a bound counted in scheduler yields; [#89](https://github.com/cboone/fosforo/issues/89) replaced it with a wall-clock deadline and `main` has been green since `6f0a860`. `actionlint` lints the workflows, pinned, with `shellcheck` beside it so the `run:` blocks are linted too, which nothing else covers. Its blind spot is stated rather than discovered: a commit-pinned action's `with:` inputs and a remote reusable workflow's are both invisible to it, so that class is caught by reading a run's warnings ([#99](https://github.com/cboone/fosforo/issues/99)) |
