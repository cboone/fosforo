...
|----------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **GPU and window server** (`smoke-appkit`, `smoke-leaks`)      | #55's measurement, [#63](https://github.com/cboone/fosforo/issues/63)'s planted leaks                                                                                                         | Two harnesses cannot both open windows and time each other's frames. `smoke-gpu` is exempt: it needs a device but no window. #63 refused the peak-RSS slope that used to be the sharpest form of this        |
...
So two issues needing the *Audio Unit* cannot be in flight at once, whatever their dependencies would allow. **What still overlaps freely is everything needing neither resource:** compiling, `zig build test`, `zig build validate-shaders`, `smoke-gpu`, documentation, and [#30](https://github.com/cboone/fosforo/issues/30), which touches no code and runs nothing on this machine.
