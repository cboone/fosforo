...
### What can be worked in parallel, and what the machine will not allow

The dependency graph permits more parallelism than this machine does, and the binding constraint is not the code. **Two resources are exclusive, and they are different resources**, so a stream needing only one of them can overlap a stream needing only the other.
...
Two consequences that are easy to miss and have already been reasoned about once:

- **#63's RSS threshold has to follow #55 rather than race it.** The pre-#55 baseline is 40.8 MB at 40 cycles rising to 68.6 MB at 400; the prediction afterwards is 100 to 130 MB. A threshold set now is obsolete on merge, and setting it competes with #55 for the same exclusive instrument. Its `smoke-leaks` half has no such problem and can land whenever.
- **#22 cannot be developed alongside anything doing host verification**, because its verification *is* installing from two worktrees to prove they are distinguishable. The issue that fixes install collisions is itself a heavy user of the contended path while being built.
