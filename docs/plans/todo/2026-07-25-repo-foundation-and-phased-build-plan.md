...
_host_ extension, fetched from `clap_host.get_extension` and consumed, so that
...
7. Tonemap pass plus palette lookup into an sRGB drawable, producing an emergent white-hot core inside a colored bloom without ever drawing a core explicitly. **Done** ([#60](https://github.com/cboone/fosforo/issues/60), [ADR 0019](../../adr/0019-brightness-is-a-fixed-transfer-function.md)). The core is executed rather than claimed: `smoke-trace` drives one deposit and thirty and reads `RGB(143, 224, 154)` and `RGB(255, 255, 255)` off the picture. The first figure read `RGB(75, 189, 96)` when #60 measured it and was corrected at [#96](https://github.com/cboone/fosforo/issues/96), because [#57](https://github.com/cboone/fosforo/issues/57) made one deposit 2.6 deposits of overlap. Four palettes ship as Zig data uploaded into a lookup, so the GPU and the model that checks it read one table.
...
| Concurrency | `zig build ring-race` and `zig build gate-race` run the history buffer and the editor's teardown gate on two threads under Thread Sanitizer, on Linux, each with a weakened control arm judged first; source canaries in `src/dsp/ring.zig` and `src/clap/gate.zig` fail `zig build test` on any machine (ADR 0016, [#90](https://github.com/cboone/fosforo/issues/90), [#91](https://github.com/cboone/fosforo/issues/91)). `Pending` and `renderer.Mailbox` keep canaries and no arm, for reasons ADR 0016's #91 amendment measures rather than asserts                                                                                                                                                                                                                                                       |
