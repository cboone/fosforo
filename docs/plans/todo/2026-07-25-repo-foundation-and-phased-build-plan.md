...
| Unit tests  | `zig build test`, 285 named tests across `src/`, in **three** optimize modes since [#94](https://github.com/cboone/fosforo/issues/94): `test` follows `-Doptimize` and is Debug by default, `test-safe` is pinned to ReleaseSafe, `test-release` to the `--release=fast` the bundle ships. Additive rather than alternatives, and the `test-modes` job runs the two pinned ones                                                                                                                                                                                                                                                                                                                                                                                                                                 |
