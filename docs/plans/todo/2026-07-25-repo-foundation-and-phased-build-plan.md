...
**[#22](https://github.com/cboone/fosforo/issues/22) took the CLAP out of that table.** REAPER honours `CLAP_PATH`, so a CLAP loads straight out of a worktree with no install step and no hash comparison to get wrong, and two streams can each run one in their own REAPER. `clap-host` is no longer the only lever there, though it stays useful for reading `clap.log` out of a release build. Neither can test resizing and neither is a load test, so both supplement Logic within an issue rather than replacing it. The Audio Unit is what did not change: it still has to land in the one shared location, so that half stays serialized, and every build now names the worktree it came from so a collision is visible rather than silent.
