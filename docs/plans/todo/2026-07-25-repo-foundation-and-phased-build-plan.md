...
7. Tonemap pass plus palette lookup into an sRGB drawable, producing an emergent white-hot core inside a colored bloom without ever drawing a core explicitly. **Done** ([#60](https://github.com/cboone/fosforo/issues/60), [ADR 0019](../../adr/0019-brightness-is-a-fixed-transfer-function.md)). The core is executed rather than claimed: `smoke-trace` drives one deposit and thirty and reads `RGB(143, 224, 154)` and `RGB(255, 255, 255)` off the picture, the first of those corrected from `RGB(75, 189, 96)` at [#96](https://github.com/cboone/fosforo/issues/96) after [#57](https://github.com/cboone/fosforo/issues/57) gave the beam area. Four palettes ship as Zig data uploaded into a lookup, so the GPU and the model that checks it read one table.
...
| Unit tests  | `zig build test`, 270 named tests across `src/`, 290 runs counting the anonymous declaration sweeps. **Debug only**, while the shipped bundle is `--release=fast`; [#94](https://github.com/cboone/fosforo/issues/94) adds the mode that ships                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
