...
## Added during execution, not in the plan as approved

**The CHANGELOG had no entry for #58**, and the plan did not catch it. That is the same class as A1: `README.md`, `AGENTS.md` and the build plan all describe velocity weighting as shipped while `CHANGELOG.md` did not mention it at all, and `AGENTS.md`'s own rule sends "what has landed" either to the phase tables or to the changelog. Four further bodies of landed work were absent for the same reason — the three optimize modes (#94), the verification program (#89 through #99), Markdown enforcement (#85, #106, #119) and the `AGENTS.md` split (#117) — and each is now one entry rather than one per issue. Every figure in them is taken from the done plan or note that measured it.

**`zig-pkg/` is the other departure**, in the opposite direction: see the struck-through E5 above. The plan proposed deleting five ignore entries and the linters refused the change within seconds of it landing.
