...
| E5  | `zig-pkg/` is ignored in `.gitignore:27`, `.prettierignore:22`, `.markdownlint-cli2.jsonc:51`, `typos.toml:48`, `.gitleaks.toml:16`, and stated as fact in `ci.yml:719`                                                                                                                                                                                | Zig 0.16 writes `.zig-cache/` and `zig-out/`; nothing in this repository has ever produced `zig-pkg/`. Five ignore lists and a workflow comment maintained for a directory that does not exist                                                                                             |
...
E5 and E6 are cheap; E1 and E2 are the two worth doing carefully, since both touch scripts that sign or delete things.
