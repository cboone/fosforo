...
| Planted defect                                            | Error returned         | What the harness printed                   | Now covered by                                                                                                                                                                                                            |
| --------------------------------------------------------- | ---------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Swap `full_scale` and `rail` in `trace_vertex`            | `LevelMisplaced`       | 0.250 read as 0.27366, off by 0.02366      | `level`, in "every level plant in the table is refused", at this exact value                                                                                                                                              |
| Apply `full_scale` twice                                  | `LevelMisplaced`       | 0.250 read as 0.22428, off by 0.02572      | The same test, second value                                                                                                                                                                                               |
| Divide x by `sample_count` rather than `sample_count - 1` | `TraceNotDrawn`        | silence lit 959 of 960 columns             | `silence`, in "refuses a line off the centre, a line that is not flat, and no line at all", one column dark                                                                                                               |
| The same, with the silence check relaxed by one column    | `TraceEndsEarly`       | three samples span columns 0 to 639 of 959 | `horizontalMapping`, in "a three-sample ramp reaches both edges, and the wrong divisor stops short"                                                                                                                       |
| Negate y                                                  | `LevelMisplaced`       | 0.250 read as -0.24897, off by 0.49897     | The same level test, third value                                                                                                                                                                                          |
| The same, with the level and rail checks skipped          | `TraceInverted`        | +0.5 sits -121.5 above centre              | `symmetry`, in "sees a negated axis and an uneven clamp as different faults"                                                                                                                                              |
| Drop the `clamp`                                          | `RailMisplaced`        | rail on row 0, expected 4.9                | `saturation`, in "the rail saturates, and dropping the clamp is what stops it"                                                                                                                                            |
| A beam colour that varies across the fragment             | `BeamNotOneColour`     | worst ray deviation 0.28152                | **Retired rather than covered.** #60 took the colour out of the deposit, so the ray this asserted no longer exists; `depositIsScalar` carries the claim that replaced it, and its own test plants one channel out of step |
| Reorder the background literal's channels                 | `BackgroundNotNeutral` | background RGBA(8, 5, 5, 255)              | `resolve`, in "the background is the palette at zero, and four ways of not being it", at these exact bytes                                                                                                                |
| Reinstate #55's `1 - decay` resolve gain                  | `ResolveNotAnAdd`      | worst channel off by -224                  | `resolve`, in "the resolve is the tonemap, and #55's gain is what that refuses". The fault is `ResolveNotTheTonemap` since #60; the claim survived the rename                                                             |
| `decay_per_frame` of 1.0                                  | `DecayWrong`           | 1.0000 of the deposit, expected 0.9000     | `decay`, in "the phosphor fades by the decay factor, and both table rows say it did not"                                                                                                                                  |
| Bind `target` rather than `source` to the decay pass      | `DecayWrong`           | 0.0000 of the deposit, expected 0.9000     | **Not coverable, and that is the better outcome.** Zig's unused-local rule refuses the defect before any judge runs. The same `decay` test covers its symptom, a deposit that never fades from zero                       |
