...
And a clamp alone could not keep the view and the drawable agreeing. REAPER calls `adjust_size`, applies the answer to the plugin's view, and shrinks its own window past it regardless. CLAP has no field anywhere carrying a smallest editor, so `clap_host_gui.request_resize` is the only other mechanism, and REAPER implements it and returns true: with the push-back the `NSView` is held at 480x270, and without it REAPER leaves the view a few points under while the drawable sits at the minimum, so the two disagree about the surface being rendered into.

**REAPER's own window still drags smaller, and that is REAPER's behaviour rather than an unfixed defect.** Established with a positive control rather than by inference: a third-party plugin with a fixed size in Logic is also freely resizable in REAPER, so REAPER does not constrain its FX window to any plugin's declared size, and nothing a plugin can send will change that. Everything CLAP offers is implemented here, `can_resize` and `get_resize_hints` and `adjust_size` and a clamping `set_size` and `request_resize`, and the window is still the host's to size. Whether another host binds its window is untested; only REAPER and Logic are available on this machine.
