...
`decay_span_nanos`' docstring argues that both arms "divide it into whole nanoseconds and span _exactly_ the same interval: 12 steps of 8 ms and 6 of 16 ms". Nothing asserts it, and the intervals live as a literal in the `for` at `src/smoke.zig:1689`. One comptime table in `verdict.zig` replaces both, read by the driver to know how many frames to run and by the judge to compose its expectation:
...
**A fourth round closed the arithmetic under both guards, and it is the one finding here whose reachability is nil.** `width * height * 4` is `usize` arithmetic in `Image.complete` and `Picture.complete`, so a product past 2^64 is illegal behaviour: a Debug panic, and in `--release=fast` a wrap that would report an undersized buffer as _complete_, which is the opposite of what the method is for. Real geometries here are 960 by 540 and overflow needs a product of 2^62, **nine trillion times larger**, so no drawable approaches it and no argument from the harness's own sizes justifies the change.
...
9. `typos`, then `ruff format --check .` and `ruff check .`. **`ruff` has no binary on this machine**: not on `PATH`, not a Homebrew formula, not in `uv tool list`. `uvx ruff@0.16.5` runs the version `ci.yml` pins. `ruff format --check` must report `1 file already formatted`; this repository's only Python file has no extension, so a run that reads _zero_ files also exits zero.
