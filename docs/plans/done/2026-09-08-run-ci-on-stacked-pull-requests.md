...
`.github/workflows/ci.yml:14` carries `branches: [main]` on its `pull_request` trigger. That filter reads the pull request's **base** ref, so a pull request targeting anything but `main` matches nothing and none of the workflow's jobs dispatch: `ci` (the reusable workflow, which expands into `Test`, `Format`, `Build` and two entries that always skip), `shaders`, `smoke`, `ring-race`, `shell`, `python`, `clap-validator` and `clap-wrapper` at the time this was written. **The count is deliberately not stated in prose**, because it moved twice while this branch was open: the phrase "the nine jobs" had stood for months against a file declaring eight, and [#99](https://github.com/cboone/fosforo/issues/99) added `actionlint` before the correction merged, making nine right again. Counts anchored to a measured run are quoted below and stay as measured. Nothing anywhere says so, and the pull request page reads green.
